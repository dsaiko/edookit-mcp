...
- **Args**: žádné
- **Env vars**: `EDOOKIT_URL`, `EDOOKIT_USER`, `EDOOKIT_PASS` (a volitelně `EDOOKIT_TIMEZONE`, `EDOOKIT_COOKIE_CACHE`, `EDOOKIT_NO_COOKIE_CACHE`, `EDOOKIT_HEADLESS_LOGIN`, `EDOOKIT_ALLOW_INSECURE_HTTP`, `EDOOKIT_HTTP_ADDR`, `EDOOKIT_HTTP_SESSION_IDLE_TTL` — viz [.env.example](.env.example))

### Vzdálené nasazení (Streamable HTTP)

Pro běžné použití přes lokální MCP host (Claude Desktop, Claude Code, Cursor…) zůstává **stdio** výchozím transportem — žádné nastavování. Pokud chcete server vystavit jako **vzdálený MCP konektor přes HTTP** (typicky za TLS reverse-proxy a OAuth gateway, např. pro ChatGPT), pusťte ho s `--http <addr>` nebo nastavte `EDOOKIT_HTTP_ADDR`:
...
| `edookit_list_courses` | Vypíše **kurzy přihlášeného učitele** — to, čemu uživatel říká *moje třídy / kurzy / skupiny* (z Hodnocení → Známkování v tabulce). Kurz = předmět vyučovaný třídě/skupině, např. „AUT - 4SA" (celá třída) a její půlené skupiny „AUT 1 - 4SA" / „AUT 2 - 4SA" (`split_group=true`). Bez argumentů vrátí jen seznam kurzů (1 request); s `course_id` jeden kurz **i s žáky** (`{study_id, name, class}`); s `include_students=true` žáky všech kurzů najednou (těžší). |
| `edookit_server_info` | Vrátí build metadata běžícího serveru jako JSON `{version, commit, build_time}`. Hodí se k ověření, **jaká verze konektoru reálně běží** (např. jestli se po rebuildu/reloadu projevila změna). Hodnoty `dev`/`none`/`unknown` znamenají lokální dev build, ne vydanou binárku. |
